...
public class OneHotEncoderTest {
  ...
  private Row[] expectedClinkOutput;
  ...
  @Test
  public void testOneHotClink() throws Exception {
    runAndCheckClink();
  }
  ...
  private void runAndCheckClink() throws Exception {
    OneHotEncoderModel model = getModel();
  ...
  private OneHotEncoderModel getModel() throws Exception {
    ...
    OneHotEncoder estimator =
        new OneHotEncoder().setDropLast(dropLast).setInputCols(inputCols).setOutputCols(outputCols);
  ...
  private void checkClinkResult(OneHotEncoderModel model) throws IOException {
    Table predictTable =
        tEnv.fromDataStream(
            env.fromElements(predictData)
                .assignTimestampsAndWatermarks(WatermarkStrategy.noWatermarks()),
    ...
    String clinkSoPath =
        rescDirPath + "/src/test/resources/feature/libperception_feature_plugin.dylib";
    String confRemotePath = "";
    String confLocalPath = rescDirPath + "/src/test/resources/feature/clink_conf";
    Table output =
        model.clinkTransform(clinkSoPath, confRemotePath, confLocalPath, predictTable)[0].select(
            $("f0"));
    ;
    Object[] actualObjects = IteratorUtils.toArray(output.execute().collect());
    Row[] actual = new Row[actualObjects.length];
    ...
    assertEquals(getFrequencyMap(expectedClinkOutput), getFrequencyMap(actual));
  }
}
